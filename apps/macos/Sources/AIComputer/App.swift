...
@MainActor final class Workspace: ObservableObject {
    ...
    func connect() {
        busy = true; error = ""; models = []; selected = ""; messages = []
        ...
        job = Task {
            ...
            do {
                ...
                models = result; selected = result.first ?? ""; connectedEndpoint = address
    ...
    func send() {
        ...
        messages = history; draft = ""; busy = true; error = ""
        let address = connectedEndpoint, model = selected
        job = Task {
            defer { busy = false }
            do {
                let answer = try await LocalClient(address).chat(model: model, messages: history)
                try Task.checkCancellation()
                messages.append(ChatMessage(role: "assistant", content: answer))
            } catch {
                messages.removeLast(); draft = text
...
enum H3Style {
    static let accent = Color(red: 0.13, green: 0.58, blue: 0.43)
    static let ink = Color(red: 0.065, green: 0.10, blue: 0.12)
...
struct ContentView: View {
    ...
    private let titles = ["홈", "대화", "음성", "이미지", "영상", "작업 보관함", "모델과 연결"]
    private let icons = ["square.grid.2x2", "bubble.left.and.bubble.right", "waveform", "photo", "film", "tray.full", "slider.horizontal.3"]
    var body: some View {
        HStack(spacing: 0) {
            sidebar
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    case 2, 3, 4:
                        ...
                        MediaView(vm: media, form: section == 2 ? speechDraft : section == 3 ? imageDraft : videoDraft, kind: MediaKind.allCases[section - 2]).disabled(vm.busy || server.running)
                    case 5: library
                    default: settings
                    }
        ...
        .onReceive(NotificationCenter.default.publisher(for: NSApplication.willTerminateNotification)) { _ in vm.stop(); media.stop(); server.stop() }
    ...
    var sidebar: some View {
        VStack(alignment: .leading, spacing: 28) {
            VStack(alignment: .leading, spacing: 4) {
                Text("H3").font(.system(size: 48, weight: .black, design: .rounded)).tracking(-3)
                ...
                ForEach(0..<7) { index in
                    ...
                    Button { section = index } label: {
                        HStack(spacing: 13) {
                            ...
                            Spacer()
                            if section == index { Circle().fill(Color(red: 0.54, green: 0.91, blue: 0.72)).frame(width: 5, height: 5) }
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text("PREVIEW 0.3 · APPLE SILICON").font(.system(size: 9, design: .monospaced)).foregroundStyle(.white.opacity(0.35))
            }.padding(14)
    ...
    var home: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 28) {
                ...
                Text("처음 사용한다면 모델과 실행 도구를 연결하세요. 이 Preview에는 모델 자동 설치가 포함되어 있지 않습니다.").font(.caption).foregroundStyle(.secondary)
            }
        }
    }
    ...
    var settings: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                    ...
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack {
                            if server.running {
                                ...
                                Button("대화 연결") { vm.endpoint = server.endpoint; vm.connect() }.buttonStyle(.borderedProminent).disabled(vm.busy)
                        ...
                        Text(server.running ? "서버 프로세스 실행 중 · 모델 준비 후 대화 연결을 눌러주세요." : "모델은 사용자가 준비한 로컬 폴더에서 불러옵니다.").font(.caption).foregroundStyle(.secondary)
                ...
                GroupBox("데이터와 작업 기록") {
                    VStack(alignment: .leading, spacing: 10) {
                        Text("대화와 작성 중인 초안은 앱을 종료하면 사라집니다. 생성 작업의 입력·설정·로그·결과는 Mac에 저장됩니다.")
    ...
    var chat: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack { Text("함께 생각해볼까요?").font(.largeTitle.bold()); Spacer(); Button("새 대화", systemImage: "square.and.pencil") { confirmClear = true }.disabled(vm.busy || vm.messages.isEmpty) }
            if media.busy { Label("콘텐츠 생성이 끝나면 대화를 이어갈 수 있습니다.", systemImage: "hourglass").foregroundStyle(.secondary) }
            if !vm.ready {
                ContentUnavailableView("대화할 AI를 연결하세요", systemImage: "bubble.left.and.bubble.right", description: Text("내 Mac에서 실행할 모델을 선택하면 이곳에서 대화를 시작할 수 있습니다."))
                Button("모델 연결하기") { section = 6 }.buttonStyle(.borderedProminent)
            } else {
                Picker("대화 모델", selection: $vm.selected) { ForEach(vm.models, id: \.self) { Text($0).tag($0) } }.disabled(vm.busy).onChange(of: vm.selected) { _, _ in vm.messages = [] }
        ...
        }.confirmationDialog("현재 대화를 지우고 새로 시작할까요?", isPresented: $confirmClear) { Button("새 대화 시작", role: .destructive) { vm.messages = [] }; Button("취소", role: .cancel) {} }
    }
