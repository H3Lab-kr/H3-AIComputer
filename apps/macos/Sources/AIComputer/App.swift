...
@MainActor final class Workspace: ObservableObject {
    ...
    init() {
        ...
        catch { self.error = L("저장한 대화를 읽지 못했습니다. 원본 파일은 보존됩니다. ") + error.localizedDescription }
    }
    func persist() {
        pendingSave?.cancel()
        ...
        catch { self.error = L("대화 저장 실패: ") + error.localizedDescription }
    }
    ...
    func connect() {
        ...
        job = Task {
            ...
            do {
                ...
                if result.isEmpty { error = L("서버에 모델이 없습니다. 실행 도구에서 로컬 모델을 준비한 뒤 다시 연결하세요.") }
                persist()
            } catch { if !Task.isCancelled { self.error = L("연결하지 못했습니다. 로컬 서버가 실행 중인지 확인하세요.\n") + error.localizedDescription } }
        }
    }
    func send() {
        ...
        job = Task {
            ...
            do {
                ...
                self.error = Task.isCancelled ? L("응답을 중단했습니다. 입력은 복원했으며 부분 응답은 아래에 표시됩니다.") : error.localizedDescription
            }
        }
    }
...
@main struct H3App: App {
    @AppStorage("app.language") private var language = AppLanguage.code
    @NSApplicationDelegateAdaptor(AppDelegate.self) var delegate
    var body: some Scene {
        Window(L("H3 · AI 컴퓨터"), id: "main") { ContentView().environment(\.locale, Locale(identifier: language)).frame(minWidth: 1000, minHeight: 700) }
...
struct ContentView: View {
    ...
    private var titles: [String] { [L("홈"), L("대화"), L("음성"), L("이미지"), L("영상"), L("작업 보관함"), L("모델과 연결"), L("AI 에이전트"), L("모델 라이브러리")] }
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                HStack {
                    ...
                    Spacer()
                    Picker("Language", selection: $language) { Text("한국어").tag("ko"); Text("English").tag("en") }.labelsHidden().frame(width: 110).accessibilityIdentifier("app-language")
                    if media.busy || vm.busy { ProgressView().controlSize(.small); Text(media.busy ? L("콘텐츠 생성 중") : L("답변 준비 중")).font(.caption) }
                    Label(vm.ready ? L("대화 연결됨") : L("로컬 워크스페이스"), systemImage: vm.ready ? "checkmark.circle.fill" : "desktopcomputer").font(.caption).foregroundStyle(H3Style.accent)
                ...
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    case 1: chat
                    case 2, 3, 4:
                        Picker(L("실행 위치"), selection: $mediaLocation) { Text(L("내 Mac · 설치 모델")).tag("local"); Text(L("온라인 API · 선택")).tag("cloud") }.pickerStyle(.segmented).disabled(media.busy || cloudMedia.busy)
                        ...
                        if server.running {
                            HStack { Label(L("콘텐츠 생성에 메모리를 사용하려면 대화 모델을 잠시 내려주세요."), systemImage: "memorychip"); Spacer(); Button(L("대화 모델 내려놓기")) { server.stop() }.disabled(vm.busy) }.font(.callout).padding(14).background(H3Style.accent.opacity(0.08), in: RoundedRectangle(cornerRadius: 12))
                        }
        ...
        .onAppear {
            agent.prepareMedia = { action in
                ...
                return L("로컬 {0} 작업 준비 완료. 실제 생성 전 사용자가 생성 화면에서 설정을 확인해야 합니다.", String(describing: kind.title))
            }
        }
    ...
    var sidebar: some View {
        VStack(alignment: .leading, spacing: 28) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(L("AI 컴퓨터")).font(.callout.weight(.medium)).foregroundStyle(.white.opacity(0.65))
            ...
            VStack(alignment: .leading, spacing: 10) {
                Label(L("내 Mac에서 실행"), systemImage: "desktopcomputer").font(.caption.weight(.medium))
                Text(L("당신의 일에 집중하는 AI.")).font(.caption).foregroundStyle(.white.opacity(0.5))
                Text("PREVIEW 0.4.1 · APPLE SILICON").font(.system(size: 9, design: .monospaced)).foregroundStyle(.white.opacity(0.35))
            }.padding(14)
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 28) {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    Text(L("대화에서 목소리, 이미지, 영상까지. H3가 내 Mac의 AI를 하나의 작업 공간으로 연결합니다.")).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
                }.padding(.vertical, 10)
                GroupBox(L("시작 준비 · 3단계")) {
                    VStack(alignment: .leading, spacing: 12) {
                        Label(L("1. Mac 확인 · {0} GB 통합 메모리", String(describing: ProcessInfo.processInfo.physicalMemory / 1_073_741_824)), systemImage: "checkmark.circle.fill")
                        Label(vm.ready ? L("2. 대화 모델 연결 완료") : L("2. 설치된 모델 또는 로컬 서버 연결"), systemImage: vm.ready ? "checkmark.circle.fill" : "circle")
                        Label(media.history.isEmpty ? L("3. 첫 콘텐츠를 만들고 보관함에서 확인") : L("3. 작업 보관함 준비 완료"), systemImage: media.history.isEmpty ? "circle" : "checkmark.circle.fill")
                        Button(vm.ready ? L("대화 이어가기") : L("모델 연결 시작")) { section = vm.ready ? 1 : 6 }.buttonStyle(.borderedProminent)
                    }.padding(12).frame(maxWidth: .infinity, alignment: .leading)
                }
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                    feature(7, L("AI에게 작업 맡기기"), L("내 자료를 읽고, 기획하고, 로컬 생성 도구를 준비하는 에이전트."), L("에이전트 시작"))
                    feature(8, L("내 모델 라이브러리"), L("설치 모델을 고르고, 최신 태그를 다운로드하고, 바로 연결하세요."), L("모델 둘러보기"))
                    feature(1, L("함께 생각하는 대화"), L("아이디어를 정리하고, 문서를 다듬고, 다음 할 일을 찾아보세요."), L("대화 시작"))
                    feature(2, L("문장을 목소리로"), L("안내 멘트와 내레이션을 만들고, 바로 들어보세요."), L("음성 만들기"))
                    feature(3, L("아이디어를 이미지로"), L("제품 사진부터 새로운 장면까지, 원하는 모습을 구체화하세요."), L("이미지 만들기"))
                    feature(4, L("장면에 움직임을"), L("첫 이미지와 장면 설명으로 소리 있는 영상을 만들어보세요."), L("영상 만들기"))
                }
                HStack(spacing: 20) {
                    Label(L("{0} GB 메모리", String(describing: ProcessInfo.processInfo.physicalMemory / 1_073_741_824)), systemImage: "memorychip")
                    Label("macOS \(ProcessInfo.processInfo.operatingSystemVersion.majorVersion)", systemImage: "desktopcomputer")
                    Spacer(); Button(L("모델과 연결 관리")) { section = 6 }
                ...
                if !media.history.isEmpty {
                    HStack { Text(L("최근 작업")).font(.headline); Spacer(); Button(L("전체 보기")) { section = 5 } }
                    ForEach(Array(media.history.prefix(3)), id: \.0.id) { r, d in historyRow(r, d) }
                }
                Text(L("모델을 연결하고 첫 결과를 만들어보세요. 대화와 생성 기록은 이 Mac에 보관됩니다.")).font(.caption).foregroundStyle(.secondary)
            }
        }
    }
    ...
    var settings: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                Text(L("내 AI를 연결하세요")).font(.largeTitle.bold())
                Text(L("직접 실행하거나, 이미 사용하는 로컬 AI 서버에 연결할 수 있습니다.")).foregroundStyle(.secondary)
                GroupBox(L("H3에서 대화 모델 실행")) {
                    VStack(alignment: .leading, spacing: 14) {
                        Text(L("설치된 mlx_vlm.server와 로컬 모델 폴더를 선택하세요.")).font(.callout).foregroundStyle(.secondary)
                        localPath(L("실행 파일"), $llmExecutable, folder: false)
                        localPath(L("모델 폴더"), $llmModel, folder: true)
                        HStack {
                            if server.running {
                                Button(L("모델 내려놓기")) { server.stop() }.disabled(vm.busy)
                                Button(L("대화 연결")) { vm.endpoint = server.endpoint; vm.connect() }.buttonStyle(.borderedProminent).disabled(vm.busy || !server.ready)
                            } else { Button(L("모델 실행")) { server.start(executable: llmExecutable, model: llmModel) }.buttonStyle(.borderedProminent).disabled(media.busy || llmExecutable.isEmpty || llmModel.isEmpty) }
                            if let dir = server.logDirectory { Button(L("로그 열기")) { NSWorkspace.shared.open(dir) } }
                        }
                        Text(server.running ? (server.ready ? L("모델 준비 완료 · 대화에 연결할 수 있습니다.") : L("모델을 준비하고 있습니다. 연결 가능 여부를 자동 확인합니다.")) : L("모델은 사용자가 준비한 로컬 폴더에서 불러옵니다.")).font(.caption).foregroundStyle(.secondary)
                        if !server.error.isEmpty { Text(server.error).foregroundStyle(.red) }
                    }.padding(12).frame(maxWidth: .infinity, alignment: .leading)
                }
                GroupBox(L("실행 중인 서버 찾기")) {
                    VStack(alignment: .leading, spacing: 12) {
                        Text(L("LM Studio · Ollama · H3 서버의 로컬 연결을 확인합니다.")).foregroundStyle(.secondary)
                        Button(vm.discovering ? L("검색 중…") : L("내 Mac에서 찾기")) { vm.discover() }.disabled(vm.discovering || vm.busy)
                        ForEach(vm.discovered, id: \.self) { address in
                            HStack { Label(address, systemImage: "checkmark.circle.fill"); Spacer(); Button(L("연결")) { vm.endpoint = address; vm.connect() }.disabled(vm.busy) }
                        }
                        if !vm.discovering && vm.discovered.isEmpty { Text(L("모델이 준비된 서버를 실행한 뒤 찾기를 눌러주세요.")).font(.caption).foregroundStyle(.secondary) }
                    }.padding(12).frame(maxWidth: .infinity, alignment: .leading)
                }
                GroupBox(L("기존 로컬 서버 연결")) {
                    VStack(alignment: .leading, spacing: 14) {
                        ...
                        HStack { TextField(L("로컬 서버 주소"), text: $vm.endpoint).textFieldStyle(.roundedBorder).disabled(vm.busy); Button(L("연결 확인")) { vm.connect() }.disabled(vm.busy) }
                        if vm.ready { Label(L("{0}개 모델 연결됨", String(describing: vm.models.count)), systemImage: "checkmark.circle.fill").foregroundStyle(H3Style.accent); Button(L("대화로 이동")) { section = 1 } }
                        DisclosureGroup(L("실행 도구 및 연결 범위")) {
                            ...
                            Text(L("앱은 루프백 주소에만 연결합니다. 외부 서버의 클라우드 연동 여부는 해당 도구의 설정에서 확인하세요.")).font(.caption).foregroundStyle(.secondary)
                        }
                    }.padding(12).frame(maxWidth: .infinity, alignment: .leading)
                }
                status
                GroupBox(L("데이터와 작업 기록")) {
                    VStack(alignment: .leading, spacing: 10) {
                        Text(L("대화와 입력 초안은 이 Mac에 자동 저장됩니다. 새 대화를 시작하면 기존 대화를 비웁니다. 생성 작업의 입력·설정·로그·결과도 Mac에 보관됩니다."))
                        Button(L("로컬 작업 폴더 열기")) { try? FileManager.default.createDirectory(at: MediaFiles.root, withIntermediateDirectories: true); NSWorkspace.shared.open(MediaFiles.root) }
    ...
    func localPath(_ title: String, _ binding: Binding<String>, folder: Bool) -> some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            HStack { TextField(title, text: binding).textFieldStyle(.roundedBorder); Button(L("찾아보기")) { let panel = NSOpenPanel(); panel.canChooseDirectories = folder; panel.canChooseFiles = !folder; panel.allowsMultipleSelection = false; if panel.runModal() == .OK, let url = panel.url { binding.wrappedValue = url.path } } }
        }.disabled(server.running)
    }
    var status: some View {
        VStack(alignment: .leading, spacing: 8) {
            if vm.busy { HStack { ProgressView().controlSize(.small); Text(L("답변 준비 중…")); Button(L("중단")) { vm.stop() } } }
            if !vm.error.isEmpty { Text(vm.error).font(.callout).foregroundStyle(.red).textSelection(.enabled) }
        }
    }
    var chat: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack { Text(L("함께 생각해볼까요?")).font(.largeTitle.bold()); Spacer(); Button(L("새 대화"), systemImage: "square.and.pencil") { confirmClear = true }.disabled(vm.busy || (vm.messages.isEmpty && vm.draft.isEmpty && vm.partialReply.isEmpty)) }
            if media.busy || agent.busy || cloudMedia.busy { Label(L("콘텐츠 생성이 끝나면 대화를 이어갈 수 있습니다."), systemImage: "hourglass").foregroundStyle(.secondary) }
            if !vm.ready {
                Label(L("모델을 연결하면 저장된 대화를 이어갈 수 있습니다."), systemImage: "bubble.left.and.bubble.right").foregroundStyle(.secondary)
                Button(L("모델 연결하기")) { section = 6 }.buttonStyle(.borderedProminent)
            }
            Group {
                if vm.ready { Picker(L("대화 모델"), selection: $vm.selected) { ForEach(vm.models, id: \.self) { Text($0).tag($0) } }.disabled(vm.busy) }
                ScrollViewReader { reader in
                    ScrollView {
                        LazyVStack(alignment: .leading, spacing: 18) {
                            if vm.messages.isEmpty {
                                VStack(alignment: .leading, spacing: 14) {
                                    Text(L("어떤 일부터 시작할까요?")).font(.title2.bold())
                                    ForEach([L("오늘 할 일을 우선순위에 따라 정리해줘"), L("새로운 서비스 아이디어를 함께 구체화해줘"), L("다음 문장을 더 명확하고 자연스럽게 다듬어줘")], id: \.self) { suggestion in
                            ...
                            ForEach(vm.messages) { m in
                                VStack(alignment: .leading, spacing: 10) {
                                    HStack { Text(m.role == "user" ? L("나") : "H3").font(.caption.bold()).foregroundStyle(H3Style.accent); Spacer(); Button { NSPasteboard.general.clearContents(); NSPasteboard.general.setString(m.content, forType: .string) } label: { Image(systemName: "doc.on.doc") }.buttonStyle(.plain).help(L("내용 복사")) }
                ...
                VStack(spacing: 8) {
                    if !vm.streamText.isEmpty || !vm.partialReply.isEmpty {
                        VStack(alignment: .leading, spacing: 8) {
                            Text(vm.busy ? L("H3 · 응답 중") : L("중단된 부분 응답")).font(.caption.bold()).foregroundStyle(H3Style.accent)
        ...
        }.confirmationDialog(L("현재 대화를 지우고 새로 시작할까요?"), isPresented: $confirmClear) { Button(L("새 대화 시작"), role: .destructive) { vm.clear() }; Button(L("취소"), role: .cancel) {} }
    }
    var library: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack { Text(L("만든 것들을 한곳에")).font(.largeTitle.bold()); Spacer(); Button(L("새로고침"), systemImage: "arrow.clockwise") { media.history = MediaFiles.history() } }
            Text(L("최신 작업부터 확인하고, 원본 결과와 실행 기록을 다시 열어보세요.")).foregroundStyle(.secondary)
            if media.history.isEmpty { ContentUnavailableView(L("아직 저장된 작업이 없습니다"), systemImage: "tray", description: Text(L("음성·이미지·영상을 만들면 이곳에 기록됩니다."))) }
    ...
    func historyRow(_ r: MediaRecord, _ d: URL) -> some View {
        HStack(spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("\(r.created, style: .date) · \(r.created, style: .time) · \(r.displayStatus)")
            }
            Spacer()
            Button(L("열기")) { media.select(r, d); section = r.input.kind == .speech ? 2 : r.input.kind == .image ? 3 : 4 }.disabled(media.busy)
            Button { NSWorkspace.shared.open(d) } label: { Image(systemName: "folder") }.help(L("Finder에서 보기"))
